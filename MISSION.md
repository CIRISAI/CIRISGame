...
**No crossing**: bonds are tubes, and two different-color tubes may not cross on a shared lattice face — a placement that would lay your bond across an opponent's existing one is illegal (DESIGN_BRIEF §4.11). It is the one indirect lever in an otherwise self-only game: you can deny an opponent a connection without ever capturing a stone. A steward with no legal placement is **forced to pass** — and only then; there is no voluntary pass. Play still fills the board, and the rare all-pass deadlock simply ends the game where it stands.
