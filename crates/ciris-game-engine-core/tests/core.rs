...
/// Drive a full game to completion with a fixed RNG policy: place a random legal
/// cell, auto-disperse, and flush owed craters at saturation. Returns the outcome.
fn play_full_game(seed: [u8; 32]) -> ciris_game_engine_core::Outcome {
...
fn same_seed_yields_identical_game() {
    ...
    for s in 0..16u8 {
        let seed = [s; 32];
        let a = play_full_game(seed);
        let b = play_full_game(seed);
